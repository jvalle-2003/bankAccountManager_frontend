...
export const rolePermissionsService = {
    ...
    async getByRole(roleId: number): Promise<RolePermission[]> {
        const response = await axios.get(`${API_URL}/role-permissions/role/${roleId}`);
        return response.data;
    },

    // Obtener asignaciones por permiso
    async getByPermission(permissionId: number): Promise<RolePermission[]> {
        const response = await axios.get(`${API_URL}/role-permissions/permission/${permissionId}`);
        return response.data;
    },

    // Asignar permiso a rol
    async create(data: RolePermission): Promise<RolePermission> {
        const response = await axios.post(`${API_URL}/role-permissions`, data);
        return response.data;
    },

    // Eliminar asignación
    async delete(roleId: number, permissionId: number): Promise<void> {
        await axios.delete(`${API_URL}/role-permissions/role/${roleId}/permission/${permissionId}`);
    }
};